...
export default function HotelDetailPage() {
      ...
      try {
        ...
        if (hotelData?.hotel_types?.length > 0) {
          const { data: similarHotelsData, error } = await supabase
            ...
            .select('*, hotel_photos(*)')
            .neq('id', hotelData.id)
            .contains('hotel_types', [hotelData.hotel_types[0]])
            .limit(4);
  ...
  const renderSidebarSimilarHotels = () => {
    if (!similarHotels || similarHotels.length === 0) return null;

    return (
      <div className="mt-6">
        <h3 className="text-lg font-semibold mb-3 flex items-center">
          <MapPin size={18} className="mr-2 text-sifnos-turquoise" />
          Similar Hotels
        </h3>
        <div className="space-y-3">
          {similarHotels.slice(0, 3).map((similarHotel) => (
            <SidebarHotelCard key={similarHotel.id} hotel={similarHotel} />
          ))}
          {similarHotels.length > 3 && (
            <Link 
              to="/hotels" 
              className="text-xs text-sifnos-deep-blue hover:text-sifnos-turquoise flex items-center mt-2"
            >
              See more hotels <ExternalLink size={12} className="ml-1" />
            </Link>
          )}
        </div>
      </div>
